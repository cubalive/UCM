...
- **Driver Live Map**: Uber-like map view in driver dashboard (`/driver`) with live GPS position marker, active trip pickup (A) / dropoff (B) markers, route polyline, ETA display, Navigate button (Apple Maps on iOS, Google Maps on Android), GPS stale indicator (>120s without GPS updates), and Map/List toggle. Endpoint: `GET /api/driver/active-trip` returns active (non-terminal) trip with route + ETA data; company-isolated.

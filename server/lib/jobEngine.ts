...
let etaTask: HarnessedTask | null = null;
let autoAssignTask: HarnessedTask | null = null;
let running = false;
...
export function startJobEngine(): void {
  ...
  etaTask = createHarnessedTask({
    name: "job_engine_eta",
    lockKey: "scheduler:lock:job_engine_eta",
    lockTtlSeconds: 30,
    timeoutMs: 60_000,
    fn: enqueueEtaCycles,
  });

  autoAssignTask = createHarnessedTask({
    name: "job_engine_autoassign",
    lockKey: "scheduler:lock:job_engine_autoassign",
    lockTtlSeconds: 30,
    timeoutMs: 60_000,
    fn: enqueueAutoAssignCycles,
  });
  ...
  registerInterval("job_engine_eta", ETA_ENQUEUE_INTERVAL_MS, etaTask, 5000);
  registerInterval("job_engine_autoassign", AUTOASSIGN_ENQUEUE_INTERVAL_MS, autoAssignTask, 8000);
}

export function stopJobEngine(): void {
  running = false;
  if (etaTask) { etaTask.stop(); etaTask = null; }
  if (autoAssignTask) { autoAssignTask.stop(); autoAssignTask = null; }
